...
def test_earth_canvas_renders(page: Page):
    ...
    error_fallback = page.locator("#earth-error")
    
    # Gracefully degrade: assert either the 3D canvas is visible or WebGL fallback error is shown
    try:
        expect(canvas).to_be_visible(timeout=3000)
        box = canvas.bounding_box()
        assert box['width'] > 0
        assert box['height'] > 0
    except AssertionError:
        expect(error_fallback).to_be_visible()

def test_calculator_realtime_updates(page: Page):
    page.goto("http://localhost:8000")
    
    # Wait for initial dashboard load to complete so there is no race condition
